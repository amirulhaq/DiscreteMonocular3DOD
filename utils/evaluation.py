...
from .post_processing.decode import ddd_decode
...
def save_results_as_json(predictions, targets, cfg):
    save_folder = cfg.EVAL.SAVE_FOLDER
    mkdir(save_folder)
    conf_thres =  cfg.EVAL.CONF_THRES
    B = predictions['heatmap'].shape[0]
    locs, rots, dims, quats, scores = ddd_decode(predictions, targets, cfg, K=40, conf_thres=conf_thres)
    for b, (loc, rot, dim, quat, score) in enumerate(zip(locs, rots, dims, quats, scores)):
        # loc, rot, dim = ddd_decode(predictions, targets, K=40, conf_thres=conf_thres)
        target_json = open(targets['label_idx'][b])
        city = os.path.normpath(targets['label_idx'][b])
        city = city.split(os.sep)
        city = city[-2]
        save_file = path_leaf(targets['label_idx'][b])
        city_dir = os.path.join(save_folder, city)
        mkdir(city_dir)
        save_json = os.path.join(city_dir, save_file)
        target_json = json.loads(target_json.read())
        camera = Camera(fx=target_json["sensor"]["fx"],
                    fy=target_json["sensor"]["fy"],
                    u0=target_json["sensor"]["u0"],
                    v0=target_json["sensor"]["v0"],
                    sensor_T_ISO_8855=target_json["sensor"]["sensor_T_ISO_8855"])
        csbox = Box3dImageTransform(camera=camera)
        obj_in_img = []
        if len(loc) > 0:
            for obj in range(loc.shape[0]):
                save, dd, ddd = {}, {}, {}
                csbox.initialize_box(size=dim[obj].cpu().numpy(),
                                    quaternion=quat[obj].cpu().numpy(),
                                    center=loc[obj].cpu().numpy(),
                                    coordinate_system=CRS_S)
                S, C, R = csbox.get_parameters(coordinate_system=CRS_V)
                amodal_box = csbox.get_amodal_box_2d()
                dd['amodal'] = [amodal_box[0], 
                                amodal_box[1], 
                                amodal_box[2] - amodal_box[0], 
                                amodal_box[3] - amodal_box[1]]
                dd['modal'] = dd['amodal']
                ddd['center'] = C.tolist()
                ddd['dimensions'] = S.tolist()
                ddd['rotation'] = [R[3], R[1], R[0], R[2]] #R.elements.tolist()
                ddd['yaw'] = rot[obj].cpu().numpy().tolist()
                save['2d'] = dd
                save['3d'] = ddd
                save['label'] = 'car'
                save['score'] = score[obj].cpu().numpy().tolist()
                obj_in_img.append(save)
            ret = {}
            ret['objects'] = obj_in_img
        # print(ret)
        else:
            ret = {}
            ret['objects'] = []
...
def read_kitti_annotations(label, classes=['car']):
    annotations = []
    TYPE_ID_CONVERSION = {
    'car': 0,
    'Cyclist': 1,
