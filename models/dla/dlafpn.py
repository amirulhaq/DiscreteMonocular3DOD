...
class BasicBlock(nn.Module):
    def __init__(self, cfg, inplanes, planes, stride=1, dilation=1):
        ...
        self.bn1 = get_norm('BN', planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
                               stride=1, padding=dilation,
                               bias=False, dilation=dilation)
        self.bn2 = get_norm('BN', planes)
        self.stride = stride
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, cfg, inplanes, planes, stride=1, dilation=1):
        ...
        self.bn1 = get_norm('BN', bottle_planes)
        self.conv2 = nn.Conv2d(bottle_planes, bottle_planes, kernel_size=3,
                               stride=stride, padding=dilation,
                               bias=False, dilation=dilation)
        self.bn2 = get_norm('BN', bottle_planes)
        self.conv3 = nn.Conv2d(bottle_planes, planes,
                               kernel_size=1, bias=False)
        self.bn3 = get_norm('BN', planes)
        self.relu = nn.ReLU(inplace=True)
        self.stride = stride
...
def build_dlaup_backbone(cfg, input_shape: ShapeSpec):
    ...
    depth_to_creator = {"dla34": dla34}
    bottom_up = depth_to_creator['dla{}'.format(34)](cfg)

    backbone = DLAUP(
        bottom_up=bottom_up,
        in_features= ['dla3', 'dla4', 'dla5'], #cfg.MODEL.DLA.DLAUP_IN_FEATURES,
        norm='BN', #cfg.MODEL.DLA.NORM,
        dlaup_node= 'conv' #cfg.MODEL.DLA.DLAUP_NODE,
    )
